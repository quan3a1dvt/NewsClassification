...
class VietnamNet(scrapy.Spider):
    ...
    def __init__(self, category=None, limit=None, *args, **kwargs):
        super(VietnamNet, self).__init__(*args, **kwargs)
        self.count = 0
    ...
    def parse(self, response):
        ...
        if self.page_limit is not None:
            if self.page_count > self.page_limit:
                return
        bntlist = response.xpath('//a[@class="panination__content-item"]')
        next_page = bntlist[len(bntlist) - 1].css('a::attr(href)').get()
        ...
        for paper_href in response.css('h3.vnn-title a::attr(href)'):
    ...
    def extract_news(self, response):
        content = response.css('div.newsFeatureBox')
        if content.css('h1.newsFeature__header-title::text').get() is not None:
            copurs =""
            for paragraph in response.css('div.maincontent p::text').getall():
                copurs+=paragraph+" "
            for paragraph in response.css('div.maincontent em::text').getall():
                copurs+=paragraph+" "          
            jsonData = {
                'label': self.label,
                'link': response.url,
                'title':content.css('h1.newsFeature__header-title::text').get(),
                'description':content.css('div.newFeature__main-textBold::text').get(),
                'content':copurs
            }
            return jsonData
